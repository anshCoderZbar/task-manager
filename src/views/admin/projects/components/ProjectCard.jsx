  ...
  return (
    <>
      {fetchProjectQuery?.isError ? (
        fetchProjectQuery?.error?.code === "ERR_NETWORK" ? (
          <ErrorAlert
          ...
          <ErrorAlert
            ...
            description="OOPS! some error occured"
          />
        )
      ) : null}
      {fetchProjectQuery?.isLoading ? (
        <div className="grid h-[60vh] w-[80vw] place-items-center">
          <LoadingIcon />
        </div>
        ...
        <ErrorAlert
          heading="Project not added"
          description="Please add project to display"
        />
      )}
    </>
  );
